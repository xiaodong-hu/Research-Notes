...
\section{A Rush Course on Fiber Bundles}
	\subsection{Fiber Bundles, Principal Bundles, and Associated Bundles}
		...
		\end{Note}
		\begin{Note}
			Such free and transitive left group action gaurantes that the mapping $G\rightarrow F$ is bijective. So $F\cong G$ and equivalently we can delete the redundant information about the fiber space $F$ in our definition and replace it with the structure group $G$ itself (so the right action is nothing but the group multiplication of $G$, which is obvious to be free and transitive). This is why in common literatures principal G-bundle is simply denoted as $E(M,G)$, and defined as ``\textbf{principal G-bundle is the differentiable fiber bundle with the fiber space coincides with the structure group $G$}''.
		\end{Note}
